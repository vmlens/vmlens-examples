            ...
            while(allInterleavings.hasNext()) {
                ...
                runParallel(() -> {
                    QueryParser parser = new QueryParser("content",  new StandardAnalyzer());
                            ...
                            // Exact match / lookup
                            docTwo.add(new StringField("id", "1", Field.Store.YES));

                            // Full-text searchable fields
                            docTwo.add(new TextField("title", "test", Field.Store.YES));


                            try {
                                indexWriter.updateDocument(new Term("id", "1"), docTwo);
                                indexWriter.commit();
                            } catch (IOException e) {
                                e.printStackTrace();
